...
A0 = Ts*[-Rs_nom/Ld_nom , Lq_nom/Ld_nom*wr(1) ; -Ld_nom/Lq_nom*wr(1) , -Rs_nom/Lq_nom];
Binv = diag([Ld_nom/Ts , Lq_nom/Ts]);
C = Ts*[0 ; -LAMpm_nom/Lq_nom*wr(1)];

uss0 = Binv*(-A0*[-10;0] - C);
s = sqrt(-(uss0'*uss0 - Vmax^2)*sc1);
...
for i = 2:Ns*0.8
    ...
    for iter = 1:2
        ...
        W = [2*B'*(eye(2) + lam(1)*Q + lam(2)*((-A0')*(Binv)'*Binv*(-A0)*sc1))*B, Ca(:,1) , Ca(:,2) , zeros(2,1);...
            Ca(:,1)' , 0 , 0 , 0;...
            Ca(:,2)' , 0 , 0 , 2*s;...
            zeros(1,2) , 0 , 2*s , 2*(lam(2) + delta)];
        
        del_x0 = -0.5*W'*grad_L;
        del_g0 = W*del_x0;
        %[V,D] = eig(W);
        %W = V*(D + max(0,delta-D))*V';
        %l = 0.1;
        l = -(del_g0'*grad_L)/(del_g0'*del_g0);
