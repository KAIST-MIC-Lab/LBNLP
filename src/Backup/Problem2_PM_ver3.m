...
wr = 1000*pi/30*PP + 1*2*1.5*0.1*5*min((2000*time - 0*4000*max(time - 3,0) + 0*2000*max(time - 4.8,0)),8000)*pi/30*PP;
%wr = min(wr,1150);
%wr = 1089.1*ones(Ns,1);
...
for i = 2:Ns*0.8
    ...
    for iter = 1:1
        ...
        %del_lam0 = -temp2*ca + 1*Ca'*grad_L/2;
        del_lam0 = -2/(temp3(1) + temp3(2))*ca + 1*Ca'*grad_L/2;
        %del_lam0 = -(temp(1) + temp(2))/2*ca + 1*Ca'*grad_L/2;
        %del_lam0 = 1*Ca'*grad_L/2;
        ...
        % temp = eig(target)
        % temp2 = min(abs(temp(1)),abs(temp(2)));
        % norm(inv(target)*temp2 - eye(2))
        % temp = eig(inv(target)*temp2) - 1
        %norm(W - eye(2))
        %temp = eig(W) - 1
