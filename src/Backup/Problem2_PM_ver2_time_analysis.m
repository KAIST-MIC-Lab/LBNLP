...
PM_avg = mean(computation_time) * 10^6;
PM_std = std(computation_time) * 10^6;
PM_up = PM_avg + 1 * PM_std;
PM_lo = PM_avg - 1 * PM_std;
x_PM = [x fliplr(x)];
inBetween_PM = [PM_up fliplr(PM_lo)];
...
plot(1:sc:length(PM_avg),PM_avg(1:sc:end),'Color',color_yellow,'LineWidth',2)
hold on
%fill(x_CM,inBetween_CM,'w','EdgeColor','none','FaceColor',color_blue,'FaceAlpha',0.4)
fill(x_PM,inBetween_PM,'w','EdgeColor','none','FaceColor',color_yellow,'FaceAlpha',0.4)
hold off
xlim([0 length(PM_avg)])
ylim([0 40])
