...
id(1) = 0; iq(1) = 0; 
u_opt_list(1,1) = 0; 
u_opt_list(1,2) = wr(1) * env.LAMpm;

%% MAIN LOOP
for i = 2:Ns*0.8
    xp = env.getObsCurrent();

    % nominal parameter
    nom_param = env.getNomParam();
    
    % reference update
    y_ref = a_LPF*Te_ref_LPF(i - 1) + (1 - a_LPF)*Te_ref(i);    
    w_ref = wr(i);

    % control decision
    [ctrl, comp_time, u_opt] = ctrl.getControl(xp, w_ref, y_ref, nom_param);
    
    % dynamics step forward
    env = env.step(u_opt, w_ref);

    % record
    env_info = env.getRealObs();
    id(i) = env_info.i(1);
    iq(i) = env_info.i(2);
    u_opt_list(i, :) = u_opt;
    Te(i) = env_info.Te;
    Te_ref_LPF(i) = y_ref;
    computation_time(i) = comp_time;
end
...
hold on
xlim([-Imax 0.5*Imax]) % MS Duplicated
ylim([-0.2*Imax Imax]) % MS Duplicated from below line
