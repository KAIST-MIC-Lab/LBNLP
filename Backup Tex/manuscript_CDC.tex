...
\author{Kyunghwan Choi and Christoph M. Hackl, \emph{Senior Member}% <-this % stops a space
\thanks{This work was supported by Korea Research Institute for Defense Technology planning and advancement (KRIT) grant funded by Korea government DAPA (Defense Acquisition Program Administration) (No. KRIT-CT-22-087, Synthetic Battlefield Environment based Integrated Combat Training Platform)}% <-this % stops a space
\thanks{K. Choi is with the School of Mechanical Engineering, Gwangju Institute of Science and Technology, Gwangju 61005, Republic of Korea {\tt\small khchoi@gist.ac.kr}}%
\thanks{C. M. Hackl is with the Hochschule M{\"u}nchen (HM) University of Applied Sciences, Laboratory of Mechatronic and Renewable Energy Systems (LMRES), Munich, 80335, Germany {\tt\small christoph.hackl@hm.edu}}%
}
...
%\KYCH{Correct. We have 8 page limit.}

\section{INTRODUCTION}

Nonlinear programming (NLP) has been used in various applications as a powerful tool for optimizing the performance of (dynamical) systems\cite{betts2010practical}. However, solving NLP problems is still challenging because of the lack of a general analytical solution and the difficulty in designing effective numerical optimization processes\cite{karamanakos2020model}. 

Typical numerical approaches for NLP are sequential quadratic programming (SQP) and augmented Lagrangian method (ALM) \cite{jorge2006numerical}. SQP solves an NLP problem effectively when the NLP can be approximated as QPs and the iteration starts near the optimal solution. However, SQP is computationally expensive, especially for large-scale problems, because it involves the inversion of the Karush-Kuhn-Tucker (KKT) matrix \cite{jorge2006numerical}. In addition, two typical methods for SQP to handle inequality constraints, the interior point method and active-set method, may require a heavy computation at each iteration and numerous iterations, respectively, when the NLP includes many inequality constraints \cite{jorge2006numerical}.
...
{\mv{c}}(\mv{x})
\end{pmatrix}^\top \mm{K}(\mv{x},\mv{\lambda}) \begin{pmatrix}
{\Delta \mv{x}}\\
{\Delta \mv{\lambda}}
\end{pmatrix},\label{eq:Delta V / Delta t}
%V := \tfrac{1}{2}g^\top g + \tfrac{1}{2}{\mm{C}_{\!\mathbb{A}}^\top}\mm{C}_{\!\mathbb{A}}.
\end{eqnarray}
%
...
\end{pmatrix} :=  - \alpha \mm{K}^\top(\mv{x},\mv{\lambda}_{\mathbb{A}})\begin{pmatrix}
...
\end{pmatrix} ,
\end{equation}
with step length $\alpha >0$ and search directions $\mv{p}_{\mv{x}}$ and $\mv{p}_{\mv{\lambda}}$ along $\mv{x}$ and $\mv{\lambda}$, respectively. 
%
...
\begin{equation}
\ddtsmall V \stackrel{\eqref{eq:Delta V / Delta t}}{\approx}  {\scriptsize \tfrac{1}{\Delta t}\begin{pmatrix}
{\mv{g}_{\!L}}\\
{\mv{c}_{\mathbb{A}}}
\end{pmatrix}^\top \!\!\!\!\mm{K} \begin{pmatrix}
{\Delta \mv{x}}\\
{\Delta \mv{\lambda}_{\mathbb{A}}}
\end{pmatrix} 
 \stackrel{\eqref{eq:update law 2}}{=} -\tfrac{\alpha}{\Delta t}\begin{pmatrix}
{\mv{g}_{\!L}}\\
{\mv{c}_{\mathbb{A}}}
\end{pmatrix}^\top \!\!\!\!\mm{K}\mm{K}^\top \begin{pmatrix}
{\mv{g}_{\!L}}\\
{\mv{c}_{\mathbb{A}}}
\end{pmatrix} \le 0.}
\end{equation}
...
{\Delta \mv{\lambda}_{\mathbb{A}}}
\end{pmatrix} 
& \!\!\!\! := \!\!\!\! &  - \alpha \underbrace{\Big[\mm{K}^\top + {\scriptsize \begin{bmatrix}
\mm{O}_{n \times n}&\mm{O}_{n \times m}\\
\mm{O}_{m \times n}&{ - 2\beta {\mm{I}_m}}
\end{bmatrix}} \Big] \begin{pmatrix}
{\mv{g}_{\!L}}\\
{\mv{c}_{\mathbb{A}}}
\end{pmatrix}}_{\scriptsize =:\begin{pmatrix}
\mv{p}_{\mv{x}}^\top,  &
\mv{p}_{\mv{\lambda}}^\top
\end{pmatrix}^\top}
\end{eqnarray}
...
%\KYCH{I usually write equations in MathType and then copy \& paste the equations into the OverLeaf. This is simply the reason and there is no special purpose. Now I have learned an efficient way of writing equations from you.}
...
\alpha [k] := \frac{\mv{x}[k]^\top \mv{p}_{\mv{x}}[k] + \mv{\lambda}_{\mathbb{A}}[k]^\top {\mv{p}}_{\mv{\lambda}}[k]}{\norm{\mv{p}_{\mv{x}}[k]}^2 + \norm{{\mv{p}}_{\mv{\lambda}}[k]}^2},
\end{align}
where $\mv{p}_{\mv{x}}$ and $\mv{p}_{\mv{\lambda}}$ denote the search directions of the respective update law (see \eqref{eq:update law 1} and \eqref{eq:update law 2}).

%\CHHA{Above, I would like to rewrite the following \begin{align}%\label{eq:alpha_selec}
%\alpha [k] := \frac{ \mv{r}[k]^\top\mv{\psi} (\mv{r}[k])}{ \norm{\mv{\psi} (\mv{r}[k])}^2},
%\end{align}
%without  
%$\mv{\psi} (\mv{r}[k]) := {\begin{pmatrix}
%{\Delta \mv{x}[k]}&{{\Delta \mv{\lambda}_{\mathbb{A}}}[k]}
%\end{pmatrix}^\top}/\alpha[k]$ ( note / check $\mv{\psi}$ does not depend on $\mv{r}$ as follows now!) and
% $\mv{r}[k] := {\begin{pmatrix}
%{\mv{x}[k]}&{{\mv{\lambda}_{\mathbb{A}}}[k]}
%\end{pmatrix}^\top}$, where I think is something wrong! I think we can ease reading if we do not introduce too many new variables. So we should only use $\Delta \mv{\lambda}_{\mathbb{A}}[k]$, etc. all those are already introduced.  Is $\psi$ really depending on $\alpha$? then there is no explicit solution as in \eqref{eq:alpha_selec}. Please double-check reference again! might it be $\alpha[k-1]$ or the left-hand side above is $\alpha [k+1]$? That is what I have written. In general check arguments above!}
%\KYCH{I have revised the above according to your comment. In fact, $\Delta\mv{x}$ and $\Delta\mv{\lambda}_{\mathbb{A}}$ contain $\alpha$; thus, dividing them by $\alpha$ was introduced to remove $\alpha$. However, the previous description was confusing as you mentioned. I think the revised version is more clear.}
...
The proposed method (update law 2), termed LBNLP, is implemented by Algorithm \ref{alg1}. This algorithm consists of the update law and thee more actions as follows:
\begin{itemize}
    \item Restrain the Lagrange multipliers for the inequality constraints greater than or equal to 0;
    \item Remove the inequality constraints with $\lambda_i = 0$ from the active set $\mathbb{A}(\mv{x})$; and
    \item Add the inequality constraints with $c_i(\mv{x}) < 0$ to the active set $\mathbb{A}(\mv{x})$.
\end{itemize}
These actions ensure that the three necessary KKT conditions for the inequality constraints, as specified in (\ref{eq:KKT3}), (\ref{eq:KKT4}), and (\ref{eq:KKT5}), are easily satisfied, despite not being explicitly incorporated into the Lyapunov function (\ref{eq:V}).

%\CHHA{UNTIL HERE I HAVE READ IN DETAIL AND REVISED THE PAPER ... I will proceed from here} 

\begin{algorithm}[!t]
\caption{LBNLP (Update law 2)} 
\label{alg1}
...
\For{$k = 0,1,2, \ldots $}
{
    ...
    Compute $\Delta\mv{x}[k]$ and $\Delta\mv{\lambda}_{\mathbb{A}}[k]$ using (\ref{eq:update law 2}) and \eqref{eq:alpha_selec}; \\
    $\mv{x}[k + 1] \leftarrow \mv{x}[k] + \Delta\mv{x}[k]$;\\
    $\mv{\lambda}_{\mathbb{A}}[k + 1] \leftarrow {\mv{\lambda}_{\mathbb{A}}[k]} + {\Delta\mv{\lambda}_{\mathbb{A}}[k]}$;\\
...
The NMPC formulation \cite{grune2017nonlinear} is 
...
& \qquad \qquad \qquad \qquad + w \, q\big( \mv{x}[k\!+\!n],\mv{u}[k\!+\!n] \big) \label{eq:NMPC1 - objective function} \\
&{\rm{subject\ to}} \nonumber\\
...
\end{align}
\end{subequations}
where arguments $k$, $k\!+\!1$, $\dots$, $k\!+\!N$ denote (succeeding) sampling instants until the prediction horizon  $1 \leq N \in \N$; $\mv{x}$, $\mv{u}$, and $\mv{y}$ denote system state, control input, and output vector, respectively; $\mv{e} := \mv{y}_{\MYREF} - \mv{y}$ represents the tracking error (difference between output $\mv{y}$ and reference signal $\mv{y}_{\MYREF}$; $\mm{F}$ and $\mm{Q}$ are symmetric positive definite matrices (of appropriate size) weighting final and preceeding tracking errors; $q(\cdot, \cdot)$ represents the control effort function; the weighting factor $w$ quantifies the relative importance of the control effort function in comparison to the tracking error terms;  
%whose typical choice is $\norm{ \mv{u} }_{\mm{R}}^2$ with symmetric positive definite matrix $\mm{R}=\mm{R}^\top >0$; 
and $\mv{f}(\cdot,\cdot)$, $\mv{g}(\cdot,\cdot)$, and $\mv{h}(\cdot,\cdot)$ denote functions of the state, output, and (in)equality constraints, respectively. 

This NMPC formulation can describe several optimal tracking control problems of various applications \cite{grune2017nonlinear}. However, it requires an appropriate tuning of the matrices $\mm{F}$ and $\mm{Q}$, and function $q$ to guarantee stability and convergence of the tracking error to zero due to the trade-off between the tracking error and control effort terms in the objective function \cite{grune2017nonlinear}. To avoid this tuning a reformulation of the problem statement is beneficial which is introduced next. 
...
\end{subequations}
%
There is no guarantee to satisfy the equality constraint in \eqref{eq:NMPC2 final e} (i) if the reference signal $\mv{y}_{\MYREF}$ cannot be attained within the prediction horizon no matter what control effort is admissible, or (ii) if a numerical optimization method for this problem does not allow violation of the equality constraint %\CHHA{REF}\KYCH{I tried to find a reference for this but I couldn't find one yet. The tracking error is hardly considered as an equality constraint, and I guess, this is why a suitable reference was difficult to find.}. 
However, if a numerical optimization method allows for the violation of constraints like the proposed LBNLP method, this NMPC problem can (approximately) be solved even if the reference signal cannot be attained within the prediction horizon. Note that SQP does not allow for the violation of constraints whereas ALM does. However, using the proposed LBNLP is even more advantageous than using ALM (recall Table \ref{tab:Comparison of LBNLP, SQP and ALM} and see validation in the next Subection~\ref{subsec:NMPTC}).

\begin{remark}
For NMPC convergence, it is beneficial if the reference signal is a smooth function as this helps to satisfy the (in)equality constraints at all sampling instants \cite{choi2021computationally}.
\end{remark}

\subsection{Nonlinear model predictive torque control (NMPTC)}
\label{subsec:NMPTC}

The NMPTC problem presented in \cite{choi2023model} is re-discussed to apply and validate the proposed LBNLP method. The NMPTC here is used to solve the Maximum-Torque per Current (MTPC) and Field Weakening (FW) problems -- subproblems of optimal feedforward torque control (OFTC) \cite{2017_Eldeeb_Aunifiedtheoryforoptimalfeedforwardtorquecontrolofanisotropicsynchronousmachines} of permanent magnet synchronous machines (PMSMs) modelled by the nonlinear dynamics
% 
\begin{equation}
	\left.
	\begin{array}{rcl}
...
Rewriting \eqref{eq:transformer-like FLDM} as current dynamics and discretizing\footnote{Invoking the Euler forward method leads to $\ddtsmall x(t) \approx (x[k\!+\!1] - x[k])/\Ts$ with sampled quantity $x[k]\approx x(k\Ts)$ at sampling instant $k\in \N$ and sampling frequency $\Ts>0$.} yields
%
\begin{equation}
	\left.
	\begin{array}{rcl}
...
&e[k\!+\!1] := \mMref[k\!+\!1] - \mM[k\!+\!1] = 0 \label{eq:NMPTC final e}
\end{align}
\end{subequations}
%
...
$$
where $\Lsd$ and $\Lsq$ denote the $d$- and $q$-axis inductances, respectively. $\psiPM$ denotes the flux linkage of the permanent magnets. The machine parameters are  
$\Rs= 25\Omega$, $\Lsd= 0.45$mH, $\Lsq=0.66$mH, $\psiPM = 0.0563$Wb, $\nP=8$, $\usmax= 56.5$V, $\ismax=70$A.
...
The NMPTC problem as stated in \eqref{eq:NMPTC} was implemented and numerically solved in MATLAB 2023a by SQP, ALM, and the proposed LBNLP, respectively. The reference torque $\mMref$ was given with 30 Nm. Two different electrical angular velocities $\omegaP$, at 840 rad/s and 1090 rad/s, were simulated to examine the cases when the inequality (voltage) constraint is inactive and active, respectively. %\CHHA{SO we do MORE THAN MTPC??? also field weakening? Before I finalize this subsection, we need to have the updated figues ... see also comment below - very nice would be to have such comparative plots for all OFTC operation strategies like MTPC, MC, FW, MTPV!}
...
ALM and the proposed method solved the problem in the formulation of (\ref{eq:NMPC2}), while SQP solved the problem in the formulation of (\ref{eq:NMPC1}) because SQP did not guarantee to handle the equality constraint (\ref{eq:NMPC2 final e}). SQP was implemented by the `\texttt{fmincon}' function available in MATLAB, which is a well-known NLP solver, with the option of `sqp'. Three different values were used for the weighting factor $w \in \{ 10^{-4},\, 10^{-2},\, 10^{0}\}$, to examine the trade-off between the tracking error terms and control effort term in the objective function. ALM was implemented by solving (\ref{eq:min_La}) using Newton's method with an iteration termination condition of $\norm{\nabla_x L_{\rm ALM}} \le 10^{-4}$. Three different values were used for the barrier parameter $\mu \in \{  10^{-2},\,  10^{0}, \, 10^{2}\}$, to examine the sensitivity of the solutions to this tuning parameter. The proposed LBNLP was implemented by Algorithm \ref{alg1}. Two different cases were examined when the for-loop in Algorithm \ref{alg1} was repeated once and twice, respectively, for each sampling instant $k$.

\begin{figure}[!t]
\centering
\subfigure[]{\includegraphics[width=0.83\linewidth]{Fig_SQP_MTPA}%
\label{Fig_SQP_MTPA}}
\vfill
\subfigure[]{\includegraphics[width=0.83\linewidth]{Fig_SQP_FW}%
\label{Fig_SQP_FW}}
\caption{NMPC results obtained using SQP with $w = 10^{-4}, 10^{-2},$ and $10^{0}$ for the conditions of (a) $w_p = 840$ rad/s  and (b) $w_p = 1090$ rad/s.
}\label{Fig_SQP}
\end{figure}

\begin{figure}%[!b]
\centering
\subfigure[]{\includegraphics[width=0.83\linewidth]{Fig_ALM_MTPA}%
\label{Fig_ALM_MTPA}}
\vfill
\subfigure[]{\includegraphics[width=0.83\linewidth]{Fig_ALM_FW}%
\label{Fig_ALM_FW}}
\caption{NMPC results obtained using ALM with $\mu = 10^{-2}, 10^{0},$ and $10^{2}$ for the conditions of (a) $w_p = 840$ rad/s  and (b) $w_p = 1090$ rad/s.
}\label{Fig_ALM}
\end{figure}

The NMPTC results obtained using SQP are depicted in Fig. \ref{Fig_SQP}. In this figure, the horizontal and vertical axes are labeled $x_1:=\isd$ and $x_2:=\isq$, representing the first and second elements of the state $\mv{x}:= (x_1,\, x_2)^\top := (\isd, \isq)^\top$, respectively. The squares represent the optimal points of the state $\mv{x}$, which are on the MTPC hyperbola when the voltage constraint is inactive (see Fig. \ref{Fig_SQP_MTPA}) and are on the inequality constraint boundary (i.e., FW operation) when the voltage constraint is active (see Fig. \ref{Fig_SQP_FW}). 
%The MTPA line, denoted by dashed lines in the figures, represents the points satisfying
%\begin{subequations}
%\begin{align}
...
%\end{align}
%\end{subequations}
%meaning the target points of the state $\mv{x}$ when the inequality constraint $h(x,u) \ge 0$ is inactive. with $w = 10^{-4}$, the state trajectory, $x_t$, satisfied the tracking condition $e_{t+N} = 0$ well but quite deviated from the control-effort-minimizing points (i.e., MTPA line) due to the large portion of the tracking error terms (see Fig. \ref{Fig_SQP}). 
The state trajectory evolves along the optimal points for $w = 10^{-2}$.  However, setting $w=10^{-4}$ or $w=10^{0}$, results in state trajectories exhibiting poor transient behavior or barely meeting the tracking condition, respectively. These outcomes were due to the small and large emphasis on the control effort term. 

The NMPTC results obtained using ALM are presented in Fig. \ref{Fig_ALM}. Similar to the SQP method, ALM achieves a desirable state trajectory exclusively with $\mu = 10^{-2}$. Smaller and larger values of $\mu$ lead to unstable trajectories and failure to meet the tracking condition, attributed to the rapid and slow updates of the Lagrange multipliers, respectively.

The NMPTC results obtained using the LBNLP method are depicted in Fig. \ref{Fig_LBNLP}. LBNLP achieves an almost instantenous tracking of the  optimal points even with only one repetition of the for-loop in Algorithm~1. Just one more repetition of the for-loop guaranteed a very accurate move. This result is significant in that conventional methods like SQP and ALM provided satisfactory results only with appropriate values of tuning parameters, whereas LBNLP guaranteed a desirable result just by repeating the simple algorithm once or twice, not relying on tuning parameters. 

Figure \ref{Fig_UL1} shows the NMPTC results obtained using update law 1. Update law 1 gives almost the same result as LBNLP when the inequality constraint is inactive (see Fig. \ref{Fig_UL1_MTPA}) but fails to handle the active constraint with one repetition of the for-loop (see Fig. \ref{Fig_UL1_FW}). With two repetitions, the state trajectory reaches the optimal points; however, it exhibits unacceptable transient behavior due to the violation of the inequality constraint. This result demonstrats the superiority of LBNLP over the existing update law 1 in terms of constraint handling.

For the condition of $\omegaP = 1090$ rad/s, the computation times were $1.76 \pm 0.20$ms for SQP with $w=10^{-2}$, $25.8 \pm 9.2 \mu$s for ALM with $\mu=10^0$, and $12.8 \pm 2.1 \mu$s for LBNLP, respectively. These results were obtained from one hundred simulation runs, where the computation time was measured using MATLAB's '\texttt{tic-toc}' function. The computation times of SQP were approximately one hundred times greater than those of ALM and the proposed method, which is probably due to the inversion of the KKT matrix for SQP and multiple iterations to obtain reasonable solutions. The average computation time of ALM was approximately twice greater than that of the proposed method. This is because ALM uses multiple iterations to guarantee the convergence of Newton's method. This result verifies that the proposed method, which is matrix-inversion-free and does not require large numbers of iterations, is computationally efficient.

\section{Conclusion}

This study presented a novel numerical optimization method to solve NLP. The method is based on a Lyapunov approach to reach the necessary conditions for optimality. The advantage of using the Lyapunov approach is that the update law can be derived in a tuning-parameter-free and matrix-inversion-free manner; thus, the proposed method can  be implemented  easily and with less iteration and computation time than conventional methods, such as SQP and ALM. The effectiveness of the proposed method was validated by using it to solve an NLP problem, which was an NMPC problem for optimal torque control of PMSMs, and comparing it with SQP and ALM. Future studies will include measurement results to also validate the proposed method experimentally.
...
%\CHHA{In the figures, the correct operation strategy should be indicated as well; at the moment, the reader might think that MTPA should work but all methods do not always converge to the MTPA hyperbola (of course, this is due to the voltage constraint, Hence, we should show FW, MTPV, etc. all "correct" points on the operation strategy line. }\KYCH{I will add such indications in the next update of the figures.} \CHHA{I would present for both angular velocities all results of SQP, ALM and LBNLP in ONE FIG. It would be nice if the simulations could show ALL OFTC operation strategies such as MPTC, MC, FW, MTPV - do you think this is feasible?}\KYCH{I think this would be the best way of presenting the comparison results. But why don't we present such figures in the journal paper? This is because, in Figs. \ref{Fig_SQP} and \ref{Fig_ALM}, SQP on ALM were simulated with three different values of tuning parameters ($w$ and $\mu$, respectively) to show their dependency on the tuning parameters. It would be quite messy if we put all the results in one figure.}

\begin{figure}[!t]
\centering
\subfigure[]{\includegraphics[width=0.83\linewidth]{Fig_LBNLP_MTPA}%
\label{Fig_LBNLP_MTPA}}
\vfill
\subfigure[]{\includegraphics[width=0.83\linewidth]{Fig_LBNLP_FW}%
\label{Fig_LBNLP_FW}}
\caption{NMPC results obtained using LBNLP (update law 2) of repeating the for-loop once and twice for each sampling instant $k$, for the conditions of (a) $w_p = 840$ rad/s  and (b) $w_p = 1090$ rad/s.
}\label{Fig_LBNLP}
\end{figure}

\begin{figure}[!t]
\centering
\subfigure[]{\includegraphics[width=0.83\linewidth]{Fig_UL1_MTPA}%
\label{Fig_UL1_MTPA}}
\vfill
\subfigure[]{\includegraphics[width=0.83\linewidth]{Fig_UL1_FW}%
\label{Fig_UL1_FW}}
\caption{NMPC results obtained using update law 1 of repeating the for-loop once and twice for each sampling instant $k$, for the conditions of (a) $w_p = 840$ rad/s  and (b) $w_p = 1090$ rad/s.
}\label{Fig_UL1}
\end{figure}
